...
class TreeMatcher(object):
    ...
    def pack(self, ud=None, version=None, src_fn=None):
        ...
        try:
            return zlib.compress(cPickle.dumps(d))
        except Exception, e:
...
class PEiDSigMatch(object):
    ...
    def load(self, fn, cache_file = None):
        ...
        c = ConfigParser.RawConfigParser({'ep_only': 'True'})
        ...
        for section in c.sections():
            ...
            # A leaf has index -2
            if -2 not in branch:
                # Create an empty list at the leaf (if this is a new leaf)
                branch[-2] = []

            # Append packer info to the leaf
            branch[-2].append((packer, ep_only))
